...
VidScribe 是一个面向视频、播客和课程音频的可移植 Agent Skill。它在私有暂存目录中完成转录，再以节省 Token 的长文本工作流重构成便于阅读和复习的中文 Markdown 总结。转录默认保留句级与词级时间轴，但最终总结仍然不包含时间戳。
...
- 输出可读转录文本、携带句级时间轴的标准化 JSON 与独立词级时间轴，同时保留服务商原始响应用于审计。
- 每个输出文件通过临时文件加原子替换提交；正常单进程运行会为同名结果选择 `-2`、`-3` 等后缀。
- 本地输入永不删除；只有本次运行创建且位于私有工作目录中的媒体才可能被清理。
- 支持时长与 Token 双重切片、逐窗保真注记、按需回查、非中文来源的中文化，以及成功后唯一总结交付。
- 入口区分两种模式：**陪看模式**（一次一视频、转录带时间戳、支持 `seek` 时间检索与提问回看、用户说「结束」后收尾）与**沉浸笔记模式**（可多视频、建议 `--no-timestamps`、直接落地）。
- `seek` 命令按时间点/区间二分定位已落盘 `*.transcript.json` 的 `segments[]`，只读本地、不联网、不建索引，供陪看问答回看使用。
- 最终交付形态为**保真讲义**：取读把每片读成近乎无损的**逐窗保真注记**，成稿按内容单元「一轮成稿 + 一轮质检」，**整篇不设字数上限**；配合 Mermaid 强触发与结构化元素门禁，使长视频总结由散文式表达升级为可验收的保真契约。
...
提交参数固定包含中英文提示 `language_hints=["zh", "en"]`，并关闭时间戳校准、保留语气词。`timestamp_alignment_enabled` 控制的是长音频上的时间戳**校准**（让识别结果与播放进度同步），不是响应中是否返回时间戳；句级与词级时间戳由服务无条件返回，因此保持 `false` 也能获得完整时间轴。`DASHSCOPE_MODEL` 仅用于兼容这些参数的 Paraformer 模型；只修改模型名称不能把该 CLI 迁移到请求结构不同的 ASR 模型。
...
按时间点定位已落盘转录中的句子（陪看回看，只读不联网）：

```bash
python skills/video-to-transcript/scripts/video_to_transcript.py seek 735000 \
  --stem source-BV1Za4y1r7KE_p2 --output-dir ./transcripts
```
...
```text
usage: video_to_transcript.py [-h] {transcribe,seek} ...

不带子命令也可直接传 source（等价于 transcribe）；seek 用于只读的时间锚定检索。
```

```text
usage: video_to_transcript.py seek [-h] [--output-dir OUTPUT_DIR]
                                   [--stem STEM] [--transcript TRANSCRIPT]
                                   [--context-before N] [--context-after N]
                                   point_ms
```

转录参数（transcribe / 默认模式）：
...
| `--no-timestamps` | 否 | 退回纯文本，不产出句级/词级时间轴（沉浸笔记模式建议使用） |
| `--segment-plan` | 否 | 另写按句子边界切片的 `*.segments.plan.json` |

检索参数（seek，只读、不联网、不建索引文档）：

| 参数 | 必需 | 说明 |
| --- | --- | --- |
| `point_ms` | 是 | 待定位的视频时间点，单位毫秒 |
| `--stem` | 否 | 工件名，如 `course` → `course.transcript.json` |
| `--transcript` | 否 | 直接指定 `*.transcript.json` 路径 |
| `--output-dir` | 否 | 工件所在目录（与 `--stem` 配合），默认 `transcripts` |
| `--context-before` / `--context-after` | 否 | 命中句前后各扩展 N 句（默认各 1），自动裁剪到边界 |

一次转录调用只处理一个输入，不会自动展开播放列表。`seek` 命中句落在句间间隙时返回间隙后的第一句；`--no-timestamps` 生成的工件（无 `segments`）会让 `seek` 明确报错「该转录无时间轴，不支持按时间检索」，而非静默返回空结果。
...
| `*.transcript.json` | `text`、句级 `segments` 时间轴，以及模型、任务 ID、来源、时长等 `metadata` | 稳定的机器处理接口 |
| `*.words.jsonl` | 词级时间轴，每行一个词，含起止毫秒与标点 | 字幕定位、逐词回溯 |
| `*.segments.plan.json` | 按句子边界规划的切片起止与覆盖区间（仅 `--segment-plan`） | 长文本分页与定位 |
| `*.asr.raw.json` | 未修改的服务商响应 | 审计、排错和未来重新处理 |

`--no-timestamps` 会退回纯文本输出，不生成 `*.words.jsonl` 且 `*.transcript.json` 不含 `segments`；`--segment-plan` 默认关闭。
...
写作入口先做**模式二选一**：陪看模式（一次一视频、保留时间戳、支持 `seek` 回看、用户说「结束」触发收尾）或沉浸笔记模式（可多视频、建议 `--no-timestamps`）。两种模式的最终稿都走同一套**保真成稿契约**：取读阶段把每个片段读成近乎无损的**逐窗保真注记**，成稿阶段每个内容单元（通常一个 H2）走**一轮成稿 + 一轮质检**，**整篇不设 800–1200 字上限**、篇幅随源内容而定，以「注记逐项覆盖 + 无冗余填充」为验收；Mermaid 命中触发即必画、未命中需自陈；结构化元素 ≥ 3。陪看模式收尾时对每个提问点追加 `## ⭐` 三段式详解。完整契约见 [`skills/video-to-transcript/references/dense-note-authoring.md`](skills/video-to-transcript/references/dense-note-authoring.md)。

避免第二次全文读取，工作流遵守以下约束：

1. 分片数取时长上限与 Token 预算计算结果的较大值；70 分钟至少均分为两个约 35 分钟片段。
2. 每片只完整阅读一次，产出近乎无损的**逐窗保真注记**：保留实体、数字、参数、步骤、命名与例子，只剔除口癖、重复与无信息过渡。
3. 后续只消费注记材料；参数缺失、步骤断裂、冲突或低置信度节点才定向回查。
4. 注记编号用于机械检查信息覆盖，发布前全部移除。
5. 最终文档按内容自然组织，并依据实际内容生成清理后的中文语义标题；同一值用于正文首行和 `.summary.md` 文件名，不使用随机来源 ID。避碰序号单独登记，`-2`、`-3` 不进入正文标题。
6. 来源主要为英文或其他非中文语言时，在事实与结构核对后执行完整的中文翻译与润色，并再次检查注记逐项覆盖。
...
- 时间戳校准保持关闭，这只会跳过长音频校准，句级与词级时间戳仍会返回并默认落盘（需要纯文本时用 `--no-timestamps`）。
